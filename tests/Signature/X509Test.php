<?php

namespace XML\Tests\Signature;

use XML\Tests\Cert;
use XML\Tests\TestCase;
use XML\Signature\X509;

class X509Test extends TestCase
{
    public function testShouldHandleP12Certificate()
    {
        $p12File = Cert::file('persona_juridica_pruebas_vigente.p12');
        $password = 'persona_juridica_pruebas';

        $newFile = X509::ensureModernP12($p12File, $password);

        // If ensureModernP12 returns null, fallback to original file
        // This happens when shell_exec is disabled and P12 is legacy
        $fileToUse = $newFile ?? $p12File;

        $this->assertNotNull($newFile, 'ensureModernP12 should not return null. Check if shell_exec is available or P12 is compatible.');

        $certs = X509::fromFile($fileToUse, $password);
        $this->assertEquals($certs->getValue(), '');

        $this->assertCount(3, $certs->all());

        // Clean up temp file if it was created
        if ($newFile !== null && $newFile !== $p12File && file_exists($newFile)) {
            @unlink($newFile);
        }
    }
}
